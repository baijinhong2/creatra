...
- **工作流**:
  1. 调 \`list_insights(limit=20)\` 拿用户最近的沉淀(感悟/项目复盘/方法论/发现/分享/碎片);按 tag / title 看哪些**和今天的热点相关**
...
- **核心哲学**:**优质内容 = 用户真实的思考 / 复盘 / 方法论 / 经验 / 发现**。AI 凭空生成的"今天 AI 又进步了"那种内容不会爆。这个 skill 是 viralpost 的**根基**——其他 skill 都会消费这里沉淀的素材
- **触发**:
  - "我做了个项目,你帮我梳理一下"
  - "我最近发现了一个 X,帮我沉淀下来"
  - "我想到一个方法论,记一下"
  - "我有个碎片想法,先存着"
  - "我看了篇文章很有启发,记一下" → sharing kind
  - 用户在聊天里说出**任何有思考密度的话**(感悟 / 复盘 / 经验 / 流程总结 / 失败教训)
  - **重要**:不一定要用户显式触发;只要聊天里有这种信号,**主动**问"要不要把这条沉淀一下?"
- **kind 选择**:
  - **reflection**(感悟):对某件事的深层理解、价值观的提炼、对行业/人生的看法
  - **project_breakdown**(项目复盘):做了个项目,从 0 到 1 的完整故事。**这个最重要**——按 起点 → 过程 → 发现/问题 → 优化 四个阶段组织
  - **method**(方法论):可复用的工作流 / SOP / 思考框架
  - **discovery**(新发现):看到一个新东西、试出一个新效果
  - **sharing**(优质内容分享):外面看到的值得转述的内容,带"为什么值得看"
  - **fragment**(碎片):一句话想法、还没成型的直觉,先存着
- **工作流**:
  1. **判断**:用户说了一段有密度的话 → 不要立刻沉淀,**先复述一次**确认理解:"我理解你想沉淀的是 XXX,对吗?"
  2. **如果用户确认 → 调 save_insight**:
     - 标题:15 字以内,直接说"是什么"(e.g. "做 viralpost 第一个 PR 死循环的教训")
     - body:**不要一字不差抄用户原话**——帮用户**结构化**。project_breakdown 必须按 起点/过程/发现/优化 整理
     - tags:2-5 个,跟账号定位相关的词
  3. **如果用户说"我刚做了个项目"** —— **这是 project_breakdown**,主动按 4 阶段反问:
     - "这个项目是怎么发现的?为什么想到做?"
     - "过程里你做了哪几步?卡在哪?"
     - "做完发现什么问题/优点?"
     - "如果再做一次,你会改什么?"
     - **逐步问**,不要一次丢 4 个问题
  4. **存完后**给一个简短的"为什么这条值得沉淀"反馈,让用户有正反馈;然后**自动**问"要不要现在基于这个沉淀给我写一条推文?"(不直接调 Skill 5,先问)
- **重要**:**主动识别信号**。哪怕用户只是在抱怨"这个 bug 调了我 2 天"——这背后可能是 reflection(调试心态)/ method(怎么定位 bug)/ discovery(某个工具好用)。要**主动**问"这个值得存下来吗?"
- **输出格式**:沉淀完成后,给一个简短的"已沉淀"确认 + 一句话鼓励,**不要写长篇分析**(用户不需要被夸奖)
  \`\`\`
  ✅ 已沉淀:《[标题]》
  kind: [kind]
  标签: [tag1, tag2, ...]
  ...
  \`\`\`
- **记忆写入**:
  - 不写 preference(insight 是独立表)
  - 调 \`save_insight(kind, title, body, tags)\`
