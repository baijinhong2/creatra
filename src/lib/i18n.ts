...
const dict = {
  ...
  'prompt.xTrends.title':     '今日 Reddit 热点',
  'prompt.xTrends.body':      '用 reddit_search 抓 Reddit 上最近 24 小时跟我所在领域相关的热门讨论。先从我 voice DNA 的关键词/方向里挑 3-5 个搜索词(用 OR 串起来,例如 "LLM agents OR autonomous agents"),每个搜最近 24h 内,按 hot 排序,挑出 score 最高的,聚类成 5-8 个热点话题。每个热点:一句话总结 + 1-2 条代表性 post 链接(r/xxx + url) + 我可以怎么蹭的角度。**只用 reddit_search,别调任何 twitter_* 工具**——X 那套已经下线了。',
  'prompt.creatorFeed.title': '看对标动态',
  'prompt.creatorFeed.body':  '用 list_creators 拿到我关注的博主/社区列表(按 weight 排,默认前 5 个)。对每个 handle: 如果是 subreddit 名字,调 reddit_get_subreddit_posts(name, sort=hot, limit=10) 拿社区热门;如果是 reddit 用户,调 reddit_get_user_posts(username, sort=new, t=week, limit=10) 拿他们本周发的帖。所有结果按时间线倒序(最新在前),每条给我:作者/社区 + 标题/时间 + 核心观点(一句话) + 互动数(score + num_comments) + reddit URL。如果某条帖子下有重要讨论,可以再调 reddit_get_post_comments(subreddit, post_id) 看 5-10 条主评论。**默认 10 条/项**——如果我说"更多"或"再来 10 条",count=20 翻页。数据量大时分批给,别一次塞太多。',
